...
cPeer::cPeer()
{
	...
	memset(AcksToSend,0,100*sizeof(unsigned long));
	nbAcks	=	0;
...
cPeer::cPeer(struct sockaddr_in ip,unsigned long id)
{
	Next		=	0;
	Previous	=	0;
...
int cPeer::SendPackets(int fd,float elapsed,fd_set master, unsigned long & nbBytes)
{
    ...
    if(FD_ISSET(fd,&fdwrite))
	{
		...
		if(nbAcks)
		{
			...
			ackPacket.Size			=	(unsigned short)(nbAcks * sizeof(unsigned long));
			...
			nbBytes += (nbAcks * sizeof(unsigned long));
...
int cPeer::PackToBuffer(char *buf,cUDPpacket *p)
{
	...
	//on pack le ID
	memcpy(buf + sent,&(p->ID),sizeof(unsigned long));
	sent += sizeof(unsigned long);
...
void cPeer::AddAck(unsigned long id)
{
