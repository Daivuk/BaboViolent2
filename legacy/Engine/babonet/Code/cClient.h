...
public:
	...
	UINT4	PendingID;			// we keep the ID of the incoming packet
	UINT4	LastPacketID;		// last emited packet ID
	...
	UINT4	NetID;			// un id unique du client
	...
	cClient();
	cClient(sockaddr_in *ip, int fileDescriptor,UINT4 netID);			//pour un serveur qui recois un client
	cClient(const char *HostIP, unsigned short port,UINT4 clientID);	//pour un client qui veut se connecter au serveur
	~cClient();
	...
	int				SendUDP(int UDPFD,UINT4 &nbByte);		//envoie les packet UDP dans la liste de UDPPacketsToSend
