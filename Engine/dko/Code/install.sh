...
if [ -f "/etc/ld.so.conf.d/babo.conf" ]; then
    if [ $EUID -ne 1000 ]; then
        ...
        cp libdko.so.0.1 $HOME/bv2_lib/
        echo "libdko.so.0.1 copied"
        ln -sf $HOME/bv2_lib/libdko.so.0.1 $HOME/bv2_lib/libdko.so.0
        ln -sf $HOME/bv2_lib/libdko.so.0 $HOME/bv2_lib/libdko.so
