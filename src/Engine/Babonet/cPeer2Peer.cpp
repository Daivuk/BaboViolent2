...
*/
	...
	if (bind(BCfd, (sockaddr *)&ipAdress, sizeof(sockaddr_in)) == -1)
	{
	...
	FD_ZERO(&master);
    FD_SET((unsigned int)(UDPfd),&master);
    FD_SET((unsigned int)(BCfd),&master);

	fdmax = std::max<int>(UDPfd,BCfd);
...
int cPeer2Peer::SendTo(INT4 peerID,cUDPpacket *newPacket,UINT4 exception)
{
	...
	if(peerID)	//si on c fait specifier un peer
	{
		...
		for(cPeer *P=PeerList;P;P=P->Next)
		{
			if(P->ID==(UINT4)peerID) //on a trouver notre peer
			{
...
cPeer* cPeer2Peer::GetPeerByID(UINT4 id)
{
	for(cPeer *P=PeerList;P;P=P->Next)
	{
		if(P->ID == id) return P;
...
void cPeer2Peer::ApplyAcks(char *ackBuffer,int bufferSize,cPeer* fromPeer)
{
	...
	UINT4 *acks = new UINT4[bufferSize/4];
...
int cPeer2Peer::AddDNSquery(char *domain,UINT4 id)
{
...
int	cPeer2Peer::DeletePeer(UINT4 peerID)
{
	for(cPeer *P = PeerList;P;P=P->Next)
	{
