...
void OpenMapDialog::Click(CControl * control)
{
	...
	if(map != lbl_mapNameItem.end())
	{
		...
		frm_mapPreview->size.set(int(mapInfo[(*map)->text].size[0] * 220.0f / max(mapInfo[(*map)->text].size[0], mapInfo[(*map)->text].size[1])), int(mapInfo[(*map)->text].size[1] * 220.0f / max(mapInfo[(*map)->text].size[0], mapInfo[(*map)->text].size[1])));
	}
}
...
void SaveAsMapDialog::LoadInfo(CString name)
{
	if(mapInfo.find(name) != mapInfo.end())
	{
		...
		frm_mapPreview->foreColor.set(1,1,1);
		frm_mapPreview->text.set("");
		frm_mapPreview->size.set(int(mapInfo[name].size[0] * 220.0f / max(mapInfo[name].size[0], mapInfo[name].size[1])), int(mapInfo[name].size[1] * 220.0f / max(mapInfo[name].size[0], mapInfo[name].size[1])));
	}
	else
	{
